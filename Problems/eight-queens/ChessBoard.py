...
class ChessBoard:
    ...
    def cost_queens(self, identifier):
        ...
        for i in range(self.n_queens):
            ...
            cost += sum(1 for c in range(self.n_dim) if board[row][c] != 0 and c != col)

            # Count queens in the same column
            cost += sum(1 for r in range(self.n_dim) if board[r][col] != 0 and r != row)
    ...
    def _construct_board_from_identifier(self, identifier):
        ...
        for i in range(self.n_queens):
            row = i // self.n_dim
            col = i % self.n_dim
            queen_number = int(identifier[i])
            if queen_number != 0:
                board[row][col] = queen_number
